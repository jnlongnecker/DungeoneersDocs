...
Each `creature` has a `base damage` determined by the major `stat` being used and the characteristics of the `weapon` they have. Whenever an `attack` says that it will `damage` the target, the "damage" being referred to is the `base damage` of the `creature`, plus any passive bonuses (like from `statuses` and `feats`).
